...
export class StreamHandler extends EventEmitter {
    ...
    public start() {
        ...
        console.log(`Connecting to stream: ${config.streamUrl}`);

        this.command = ffmpeg(config.streamUrl)
    ...
    private processAudioData(chunk: Buffer) {
    ...
    private flushBuffer() {
        ...
        if (totalLength < config.minDuration * 32000) {
            this.buffer = [];
            return;
        }
        ...
        const filename = path.join(process.cwd(), 'temp_chunks', `chunk_${timestamp}.wav`);
